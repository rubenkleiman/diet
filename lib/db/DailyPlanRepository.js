...
class DailyPlanRepository {
    ...
    async getDetails(planId, userId) {
        const data = await this.getDetails1(planId, userId);
        await this.getDetails2(data);
    ...
    `
        ...
        results.forEach(nutrition => {
            ...
            menu.type = nutrition.type;
            const vals = this.nutritionalValues(nutrition)
            if (menu.totals) {
    ...
    nutritionalValues(nutrition) {
        ...
        const recipeAmtInGrams = this.toGrams(nutrition.recipeAmount, nutrition.recipeUnit, nutrition.density);
        const ingredientAmtInGrams = this.toGrams(nutrition.ingredientAmount, nutrition.ingredientUnit, nutrition.density);
        ...
        const data = {
            ...
            polysaturated_fat: safeVal(nutrition.polysaturated_fat, servingFactor),
     ...
     */
    async getDetails1(planId, userId) {
        let sql = `
    ...
    async create(request, userId) {
        ...
        try {
            ...
            let itemOrder = 1
            for (const menu of dailyPlanMenus) {
                await database.run(`
