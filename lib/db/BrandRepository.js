...
class BrandRepository {
    ...
    async create(brandData, userId) {
        ...
        try {
            ...
            const brandResult = await database.run(`
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [userId, brandData.name, brandData.serving, brandData.servingUnit, brandData.density || null, brandData.oxalatePerGram, 'mg/g'])
            ...
            // Format nutrition values - add unit suffix if value exists
            const formatValue = (val) => val ? `${val} mg` : null
    ...
    async update(brandId, brandData, userId) {
        ...
        try {
            ...
            `, [brandData.name, brandData.serving, brandData.servingUnit, brandData.density || null, brandData.oxalatePerGram, brandId, userId])
