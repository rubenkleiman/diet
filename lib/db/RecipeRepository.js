...
class RecipeRepository {
    ...
    async calculateRecipeNutrition(request, userId) {
        ...
        for (const item of items) {
            ...
            oxalateMg += item.brandData.oxalatePerServing * servingFactor;
        }
    ...
    _validateRecipeNutritionRequest(request) {
        if (!request?.ingredients?.length) {
            throw Error("Request missing list of ingredients.");
        }
        request.ingredients.forEach(i => {
            if (typeof i != "object") {
                throw Error(`Item ${i} in ingredient list must be an object.`);
            }
            const keys = Object.keys(i);
            if (keys.length != 3) {
                throw Error(`Item ${i} in ingredient list must have three elements. ${JSON.stringify(request)}`);
            }
            if (!i.brandId || !i.amount || !i.unit) {
                throw Error(`Ingredient ${JSON.stringify(i)} missing brandId, amount, or unit property.`);
            }
            if (!["g", "ml"].includes(i.unit)) {
                throw Error(`Unit ${JSON.stringify(i)} must be "g" or "ml".`);
            }
        });
    }
