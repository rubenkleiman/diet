...
class BrandRepository {
    ...
    /**
     * Provide nutrient data.
     */
    async getNutrients() {
        const sql = `
            select key, display_name as displayName, unit, category, dash_relevant as dashRelevant, item_order
            from nutrients order by item_order
        `;
        let nutrients = await database.all(sql, []);
        nutrients.forEach(n => n.dashRelevant = n.dashRelevant ? true : false);
        return { nutrients }
    }
    ...
    async getById(id, userId = null) {
        ...
        // TODO get field names from getNutritionData()
        let sql = `
            SELECT 
                b.*,
                bd.calories,
        ...
        if (!brand) {
            throw Error(`Brand id ${id} not found.`)
        }
        ...
        let gramsPerServing = brand.serving;
        if (brand.serving_unit == "ml") {
            if (!brand.density) {
                throw Error(`Ingredient brand id ${brand.id} does not have density but serving unit is expressed as ml.`)
            }
            gramsPerServing *= brand.density;
        }

        // Build nutrition data object
        const data = {}
        const nutritionFields = [ // TODO get field names from getNutritionData()
            'calories', 'sodium', 'cholesterol', 'sugars', 'protein', 'dietary_fiber',
            'carbohydrates', 'calcium', 'potassium', 'magnesium', 'selenium',
            'manganese', 'zinc', 'iron', 'fat', 'saturated_fat', 'polyunsaturated_fat',
    ...
    async create(brandData, userId) {
        ...
        try {
            ...
            `, [
                ...
                formatValue(data.polyunsaturated_fat),
