...
// Updated to read oxalates from brands.json
...
class IngredientsManager {
    ...
    getCompactInfo(brand) {
        ...
        // Get oxalate content (per gram) from brand.oxalatePerGram
        let oxalatePerGram = 0
        if (brand.oxalatePerGram) {
            oxalatePerGram = parseFloat(brand.oxalatePerGram)
        }
    ...
    getIngredientDetails(brandName) {
        ...
        // Get oxalate per gram from brand.oxalatePerGram
        let oxalatePerGram = 0
        if (brand.oxalatePerGram) {
            oxalatePerGram = parseFloat(brand.oxalatePerGram)
        }
