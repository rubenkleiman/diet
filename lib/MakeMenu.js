...
class MakeMenu {
    ...
    /**
     * Gets list of all recipe names
     * @returns {Array<string>} Array of recipe names
     */
    getAllRecipeNames() {
        const recipeNames = []
        for (const name in Recipes._recipes) {
     ...
     */
    getRecipeData(recipeName, summary = false) {
        const recipe = Recipes.find(recipeName)
        if (!recipe) {
            return null
        }
     ...
     */
    getRecipeComparison(recipeName, variations = {}, addedIngredients = {}, removedIngredients = [], summary = false, explained = []) {
        const recipe = Recipes.find(recipeName)
        if (!recipe) {
            return null
        }
        ...
        for (const ingredientName of Object.keys(allIngredients)) {
            const brand = Brands.find(ingredientName)
            if (!brand) {
                throw new Error(`Ingredient "${ingredientName}" not found in brands.json`)
            }
        }
