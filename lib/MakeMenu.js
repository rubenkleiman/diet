// FIXED 2025-10-31 - Updated to use database repositories instead of JSON files
import Converter from "./Converter.js";
import RecipeRepository from "./db/RecipeRepository.js";
...
class MakeMenu {
     ...
     */
    async getRecipeData(recipeName, summary = false, userId = null) {
    ...
    async calculateRecipe(recipe, includeDetails = false, trackContributions = [], userId = null) {
        ...
        for (const [brandName, measure] of Object.entries(recipe.ingredients)) {
            const brand = await this.brandRepository.getByName(brandName, userId);
            if (!brand) {
    ...
    async getRecipeComparison(recipeName, variations = {}, addedIngredients = {}, removedIngredients = [], summary = false, explained = [], userId = null) {
        ...
        for (const ingredientName of Object.keys(allIngredients)) {
            const brand = await this.brandRepository.getByName(ingredientName, userId);
            if (!brand) {
